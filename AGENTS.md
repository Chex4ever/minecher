...
- `server/src/services/imports.ts` — `ImportService`: `importPath` (copy folder → staging → `servers.create()` → atomic rename), `importMcS` (`.mcs` zip → `adm-zip` extract), `exportMcS` (zip with `mcs.json` manifest), `listRemote`/`importRemote` (pull: `fetch` login → list `/api/servers` → download `/api/servers/:id/export` stream → `importMcS`), `pushRemote` (push: export local server → stream multipart upload to remote `POST /api/imports/mcs`). Auto-detects type from jar/dir layout; type `custom` skips jar download.
...
- `POST /api/imports/path` (import from folder), `POST /api/imports/mcs` (multipart `.mcs`), `GET /api/servers/:id/export` (`.mcs` download), `POST /api/imports/remote/list` (list servers on another Minecher), `POST /api/imports/remote` (pull: remote login → download export stream → import), `POST /api/imports/remote/push` (push: export local server → stream multipart upload to remote `/imports/mcs`). Server types include `custom` (no jar download; existing jar required). Export stream: async handler must `return` the stream — calling `reply.send(stream)` without returning makes fastify `wrapThenable` send `undefined` again (0-byte response + `stream closed prematurely`).
