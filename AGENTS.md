...
- **IPv6 is banned.** Server binds IPv4 only (`0.0.0.0`), vite binds `127.0.0.1`, `dns.setDefaultResultOrder("ipv4first")` in `server/src/index.ts`, and `MC_HOST` containing `:` is rejected in `server/src/config.ts`. Do not reintroduce IPv6 or `localhost`-with-IPv6 resolution in new code.
...
- `server/src/index.ts` wires services into an `AppContext` (`server/src/services/context.ts`) and registers route groups.
...
- `server/src/versions/*` — `VersionSource` adapters (vanilla/paper/spigot/forge/fabric) + `DownloadService` cache in `data/versions/`.
...
- `GET /api/ports/:port` — availability probe (TCP bind + managed running servers).
...
data/
  ...
  servers/<id>/            java process working dir: server.jar, world/, server.properties, logs/
...
- `server.properties` is synthesized on every start (existing keys preserved; `online-mode=false` default overridable via settings; `server-port`/`motd` forced from the record) and `eula.txt` is written with `eula=true`. `GET /api/servers/:id` adds `serverPropsFile` (effective on-disk props) for the Settings tab.
